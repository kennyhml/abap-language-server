...
var root_6 = /* @__PURE__ */ from_html(`<p class="not-found svelte-17y3m0f">No connections found. Follow <a href="https://github.com/kennyhml">the instructions</a> to add a connection provider.</p>`);
var root_8 = /* @__PURE__ */ from_html(`<p class="not-found svelte-17y3m0f">No connection matches this filter - please check again.</p>`);
...
function ConnectionList($$anchor, $$props) {
  push($$props, true);
  let mappedConnections = /* @__PURE__ */ user_derived(() => $$props.connections.toSorted((a, b) => a.systemId.localeCompare(b.systemId)).map((v, i) => ({ ...v, id: i })));
  let searchFilter = /* @__PURE__ */ state("");
  let matchingEntries = /* @__PURE__ */ user_derived(() => getMatchingConnections(get(mappedConnections), get(searchFilter)));
  let selectedId = /* @__PURE__ */ state(-1);
  function getMatchingConnections(conns, filter) {
    if (!filter) {
      return conns;
    }
    let lowerFilter = filter.toLowerCase();
    return conns.filter((conn) => conn.systemId.toLowerCase().includes(lowerFilter) || conn.name.toLowerCase().includes(lowerFilter));
  }
  ...
  function noneMatchFilter() {
    return get(mappedConnections).length !== 0 && get(matchingEntries).length === 0;
  }
  var div = root$1();
  var node = child(div);
  TextInput(node, {
  ...
  each(tbody, 21, () => get(matchingEntries), (conn) => conn.id, ($$anchor2, conn) => {
    var tr = root_1();
  ...
  var node_3 = sibling(table, 2);
  {
    ...
    var alternate_2 = ($$anchor2) => {
      ...
      var node_4 = first_child(fragment);
      {
        ...
        if_block(
          node_4,
          ($$render) => {
            if (noneMatchFilter()) $$render(consequent_3);
          },
          true
        );
      }
    ...
    if_block(node_3, ($$render) => {
      if ($$props.connections.length === 0) $$render(consequent_2);
...
function AddConnection($$anchor, $$props) {
  push($$props, true);
  let connections = /* @__PURE__ */ state(proxy([]));
  ...
  window.addEventListener("message", (event) => {
    ...
    if (((_a2 = event.data) == null ? void 0 : _a2.type) === "init") {
      set(connections, event.data.data.connections, true);
